			...
			foreach ($shipping_methods as $shm)
			{
				...
				if ($default_date)
				{
					...
					if (count($delivery_dates))
					{
						...
						if ($delivery = $cart_bpost->getDeliveryCode($shm))
							...
							else
							{
								...
								if ($def_is_sat)
								{
									...
									$store_cents = (int)$sat_info['cost'] * 100;
								}

								$cart_bpost->setDeliveryCode($shm, $store_date, $store_cents);
							}
						...
						{
							$cents = $date_service->isSaturday($dt) ? (int)$sat_info['cost'] * 100 : 0;
							$has_cost = $has_cost || $cents > 0;
							$date_key = PsCartBpost::intEncodeDeliveryCode($dt, $cents);
