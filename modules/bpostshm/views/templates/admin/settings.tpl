...
<div id="bpost-settings">
	...
	<form class="form-horizontal{if $version < 1.5} v1-4{elseif $version < 1.6} v1-5{/if}" action="#" method="POST" autocomplete="off">
		<fieldset class="panel" id="fs-account">
			<div class="panel-body">
				...
				<p><span class="label label-danger red">{l s='Warning' mod='bpostshm'}</span>:  {l s='Delivery option settings label management WARNING' mod='bpostshm'}.</p>
				<p>
					<a href="{l s='http://bpost.freshdesk.com/support/solutions/folders/208531' mod='bpostshm'}" title="{l s='Documentation' mod='bpostshm'}" target="_blank">
						<img src="{$module_dir|escape}views/img/icons/information.png" alt="{l s='Documentation' mod='bpostshm'}" />{l s='Documentation' mod='bpostshm'}
					</a>
		...
		{if isset($store_details_info)}
			...
			{assign var="sd_id" value="sd-$key"}	
				<div class="form-group">
					...
					{else}
						<input type="text" name="{$sd_id|escape}" id="{$sd_id|escape}" value="{$details.value|escape}" data-sdkey="{$key|escape}" size="50" maxlength="{$details.max|intval}" {if $details.required}required="required" placeholder="{l s='This field cannot be empty' mod='bpostshm'}" data-invalid="{l s='This field cannot be empty' mod='bpostshm'}"{/if} />
	...
	$(function() {
		...
		$('input#num-dates').integerInput({ min: 2, max: 7 });
...
{/literal}
		...
		//
		$('button[name="submitAccountSettings"]').on('click', function(e) {
{literal}
			var js_storedet = {};
{/literal}
			$('[data-sdkey]').each(function() {
				var elm = $(this),
					key = elm.data('sdkey'),
					is_required = elm.prop('required'),
					text = elm.val().trim();

				if (is_required && '' == text) {
					e.preventDefault();
					
					var sup = elm.next('sup'),
						warner = $('<span>'),
						warn_secs = 2.0;

					sup.addClass('super-sup');
					warner
						.text(elm.data('invalid'))
						.addClass('hammered');
					sup.after(warner);
					...
					}, parseInt(1000*warn_secs));
					
					return false;
				}

				js_storedet[key] = text;
			});

			$jsn_list = JSON.stringify(js_storedet);
			$('input[name="store_details"]').val($jsn_list);
		});
		//
		...
		// $('.delopt-tabs').idTabs();
		//
		$('#del-set-deps').toggle($('input[name="display_delivery_date"]').prop('checked'));
		$('input[name="display_delivery_date"]').on('change', function() {
			$('#del-set-deps').toggle(this.value);
		});
		$('#choose-date-deps').toggle($('input[name="choose_delivery_date"]').prop('checked'));
		$('input[name="choose_delivery_date"]').on('change', function() {
			$('#choose-date-deps').toggle(this.value);
		});
		$('button[name="submitDeliverySettings"]').on('click', function() {
			elm_hour = $('input#cutoff-time');
			elm_hour.val(elm_hour.val().replace(/:/g, ''));
		});
		$('#label-deps').toggle($('input[name="label_use_ps_labels"]').prop('checked'));
		$('input[name="label_use_ps_labels"]').on('change', function() {
			$('#label-deps').toggle(this.value);
		});
		// bpost-tabs
